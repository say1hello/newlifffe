...
class ExcelExport
{
            ...
            switch ($objects[$i]->category) {
                case "1":
                    switch ($objects[$i]->rooms) {
                        case "1":
                            $this->setSheetContent($excel, "Однушки", $object_use, 1);
                            break;
                        case "2":
                            $this->setSheetContent($excel, "Двушки", $object_use, 2);
                            break;
                        case "3":
                            $this->setSheetContent($excel, "Трешки", $object_use, 3);
                            break;
                        default:
                            $this->setSheetContent($excel, "Четырешки", $object_use, 4);
                            break;
                    }
                    break;
                case "2":
                    $this->setSheetContent($excel, "Дома", $object_use, 5);
                    break;
                case "3":
                    $this->setSheetContent($excel, "Комнаты", $object_use, 0);
                    break;
                default:
                    break;
            }
    ...
    public function setSheetContent($excel, $sheet_name, $object, $obj_type)
    {
        $excel->sheet($sheet_name, function ($sheet) use ($object, $obj_type) {
            $sheet->row($this->count[$obj_type], array(
                $object["area"],
                $object["street"],
                $object["price"],
                $object["desc"],
