...
class ObjectsRepository extends Repository
{
    ...
    public function __construct(
        Subject $object,
        ImagesRepository $i_rep,
        ComfortsRepository $c_rep,
        AreasRepository $a_rep
    ) {
        $this->model = $object;
        $this->i_rep = $i_rep;
        $this->c_rep = $c_rep;
    ...
    public function addObject($request, $area = null)
    {
        if ($request->has("obj_type")) {
            ...
            switch ($obj_type) {
                case "1" :
                    $this->model->category = $request->obj_type;
...
//                }
    ...
    public function getAlias($object)
    {
        switch ($object->category) {
            case "1":
                $title = $object->rooms . "-к квартира " . $object->square . " м " . $object->floor . "-" . $object->build_floors . " эт. " . $object->id;
                return $this->transliterate($title);
                break;
            case "2":
                $title = $object->type . " " . $object->home_square . " м на участке " . $object->earth_square . " сот. " . $object->id;
                return $this->transliterate($title);
                break;
            case "3":
                $title = "комната " . $object->square . " м  в " . $object->rooms . "-к " . $object->floor . "-" . $object->build_floors . " эт. " . $object->id;
                return $this->transliterate($title);
                break;
            default:
                break;
    ...
    private function searchObject($data, $pagination, $query)
    {
        ...
        switch ($data["category"]) {
            case "1":
                $square_min = ($data["square_1_min"] == 10) ? 1 : $data["square_1_min"];
                ...
                break;
            case "2":
                $square_min = ($data["square_2_min"] == 10) ? 1 : $data["square_2_min"];
    ...
    public function getObjImage($object)
    {
        switch ($object->category) {
            case "1":
                $image = ($object->rooms > 5) ? "flat-6.svg" : "flat-" . $object->rooms . ".svg";
                break;
            case "2":
                $image = "house.svg";
                break;
    ...
    public function getTitle($object)
    {
        switch ($object->category) {
            case "1":
                $title = $object->rooms . "-к квартира " . $object->square . " м² " . $object->floor . "/" . $object->build_floors . " эт.";
                break;
            case "2":
                $title = $object->type . " " . $object->home_square . " м² на участке " . $object->earth_square . " сот.";
                break;
            case "3":
                $title = "Комната " . $object->square . " м²  в " . $object->rooms . "-к " . $object->floor . "/" . $object->build_floors . " эт.";
                break;
            default:
                $title = "";
                break;
