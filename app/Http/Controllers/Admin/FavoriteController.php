...
class FavoriteController extends AdminController
{
    ...
    public function index(JavaScriptMaker $jsmaker)
    {
        ...
        $this->content = view(config('settings.theme') . '.admin.favorites')->with(array(
            "favorites" => $favorites,
            "a_favorites" => $a_favorites,
            "actions" => $actions
        ))->render();
    ...
    public function Favorite(Request $request, Subject $object)
    {
        $this->checkUser();
        if ($request->type == "add") {
