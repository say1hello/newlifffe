                ...
                @foreach($objects as $object)
                    <tr>
                        <td class="table-check">
                            <div class="checkbox checkbox-only">
                                <input class="action-checkbox" name="objects[]" id="tbl-check-{{$object->id}}"
                                       type="checkbox" value="{{$object->id}}">
                        ...
                        <td class="table-obj">
                            <div class="tab_content">
                                @if($object->category == 1)
                                    <a href="{{route('site.object',['object'=>$object->alias])}}">{{$object->rooms}}-к
                                        квартира</a><br>{{$object->square}} м² {{$object->floor}}
                                    /{{$object->build_floors}} эт.<br>{{ $object->created_at->format('d/m/Y') }}
                                @elseif($object->category == 2)
                                    <a href="{{route('site.object',['object'=>$object->alias])}}">{{$object->type}}</a>
                                    <br>{{$object->home_square}} м² на участке {{$object->earth_square}}
                                    <br>{{ $object->created_at->format('d/m/Y') }}
                                @elseif($object->category == 3)
                                    <a href="{{route('site.object',['object'=>$object->alias])}}">Комната
                                        в {{$object->rooms}}-к</a><br>{{$object->square}} м² {{$object->floor}}
                                    /{{$object->build_floors}} эт.<br>{{ $object->created_at->format('d/m/Y') }}
                                @endif
                            </div>
                        </td>
                        <td class="table-address">
                            <div class="tab_content">{{ $object->address }}
                                ,<br>{{ str_replace(array("микрорайон", "улица", "Квартал", "квартал", "поселок"), array("мкр", "ул", "кв-л", "кв-л", "п"), isset($object->raion) ? $object->raion->name : "" ) }}
                                , <br> {{ $object->gorod->name }}</div>
                        </td>
                        <td>
                            <div class="tab_content">{{ number_format($object->price) }}</div>
                        </td>
                        <td class="table-desc">
                            <div class="tab_content">{!! $object->desc !!}</div>
                        </td>
                        <td>
                            <div class="tab_content">{{ number_format($object->surcharge) }}</div>
                        </td>
                        @if($type == "outed")
                        ...
                        @else
                            <td class="table-comment">
                                <div class="tab_content">{{ $object->comment }}</div>
                            </td>
                        @endif
                        <td class="table-contact">
                            <div class="tab_content">
                                <a href="{{route('object.phone', ['object'=>$object->alias])}}" data-show="false"
                                   data-id="{{$object->id}}" class="btn btn-success btn-phone js-show-phone col-md-12">
                                    <span class="button-text js-name">Показать</span> <span
                                            class="button-text js-father_name"></span><br>
                            ...
                            @if($object->calls->isNotEmpty() && ($type == 'inwork' || $type == 'prework' || !$object->working_id))
                                <!-- Button trigger modal -->
                                    <button type="button" class="btn btn-primary btn-sm" data-toggle="modal"
                                            data-target="#calls-{{$object->id}}">
                                        Звонки
                                    </button>
                                    <!-- Modal -->
                                    <div class="modal fade" id="calls-{{$object->id}}" tabindex="-1" role="dialog"
                                         aria-labelledby="calls-{{$object->id}}Label" aria-hidden="true">
                                                    ...
                                                    @foreach($object->calls as $call)
                                                        ...
                                                        <div class="col-12">
                                                            <audio style="width: 100%" controls preload="none">
                                                                <source src="{{route('call.get',[ 'data' => $call->exec_at->format('Y-m-d'),'url'=>$call->url])}}"
                                                                        type="audio/mpeg">
                                                            </audio>
                                                        </div>
                                                    @endforeach
                        ...
                        <td class="table-actions">
                            <div class="tab_content">
                                <div class="btn-actions centovka">
                    ...
                    @if($objects->lastPage() > 1)
                        @if($objects->currentPage() !== 1)
                            ...
                            <li class="page-item"><a class="page-link"
                                                     href="{{ $objects->url(($objects->currentPage() - 1)) }}">&lsaquo;</a>
                            </li>
                        @else
                        ...
                        @if($objects->currentPage() !== $objects->lastPage())
                            <li class="page-item"><a class="page-link"
                                                     href="{{ $objects->url(($objects->currentPage() + 1)) }}">&rsaquo;</a>
                            </li>
                            <li class="page-item"><a class="page-link" href="{{ $objects->url($objects->lastPage()) }}">&raquo;</a>
                            </li>
                        @else
